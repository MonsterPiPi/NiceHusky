...
public class LoginController {
    ...
    private String loginUser(Userlogin user) {
        if (isRelogin(user)) return "SUCC"; // 如果已经登陆，无需重新登录
        return shiroLogin(user); // 调用shiro的登陆验证
    }
    private String shiroLogin(Userlogin user) {

        // 组装token，包括客户公司名称、简称、客户编号、用户名称；密码
        UsernamePasswordToken token = new UsernamePasswordToken(user.getLoginName(), user.getPwd().toCharArray(), null);
    ...
    private boolean isRelogin(Userlogin user) {
